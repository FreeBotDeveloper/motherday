...
if (bgMusic) {
    // Fetch Audio Metadata (ID3 Tags)
    const audioSource = bgMusic.querySelector('source');
    if (audioSource && audioSource.src) {
        if (window.jsmediatags) {
            window.jsmediatags.read(audioSource.src, {
                onSuccess: function(tag) {
                    const title = tag.tags.title;
                    const artist = tag.tags.artist;
                    let displayStr = "Background Music";
                    if (title && artist) displayStr = `${artist} - ${title}`;
                    else if (title) displayStr = title;
                    else {
                        let filename = audioSource.src.split('/').pop();
                        displayStr = decodeURIComponent(filename).replace('.mp3', '');
                    }
                    const titleEl = document.querySelector('.music-title');
                    if (titleEl) titleEl.innerText = displayStr;
                },
                onError: function(error) {
                    console.log("ID3 tag read error:", error);
                    let filename = audioSource.src.split('/').pop();
                    let displayStr = decodeURIComponent(filename).replace('.mp3', '');
                    const titleEl = document.querySelector('.music-title');
                    if (titleEl) titleEl.innerText = displayStr || "Background Music";
                }
            });
        }
    }
    ...
    bgMusic.addEventListener('timeupdate', () => {
        if (bgMusic.duration) {
            musicProgress.value = (bgMusic.currentTime / bgMusic.duration) * 100;
            musicTimeDisplay.innerText = `${formatTime(bgMusic.currentTime)} / ${formatTime(bgMusic.duration)}`;
        }
    });

    // Load initial duration
    bgMusic.addEventListener('loadedmetadata', () => {
        musicTimeDisplay.innerText = `0:00 / ${formatTime(bgMusic.duration)}`;
    });
...
function startExperience() {
    if (!interactionStarted) {
        ...
        initAudio();
        document.getElementById('wish-trigger-btn').classList.add('visible');
        if (bgMusic) {
...
// ---- Floating Wishes Logic ----
const wishTriggerBtn = document.getElementById('wish-trigger-btn');
const wishModal = document.getElementById('wish-modal');
const closeWishBtn = document.getElementById('close-wish-btn');
const releaseWishBtn = document.getElementById('release-wish-btn');
const wishInput = document.getElementById('wish-input');
const lanterns = [];

const lanternSlots = [
    {x: -2.8, y: 1.5},  {x: 2.8, y: 1.5},   // Top edges
    {x: -2.8, y: 0},    {x: 2.8, y: 0},     // Middle edges 
    {x: -2.8, y: -1.5}, {x: 2.8, y: -1.5}   // Bottom edges
];
let availableSlots = [...lanternSlots];

wishTriggerBtn.addEventListener('click', (e) => {
    e.stopPropagation();
...
releaseWishBtn.addEventListener('click', (e) => {
    e.stopPropagation();
...
function createLantern(text) {
    ...
    ctx.fillStyle = '#4a2c11'; // Darker text for extreme clarity
    ctx.font = 'bold 40px ' + (currentLang === 'th' ? 'Prompt' : 'Playfair Display'); // Larger font
    ...
    const startY = (canvas.height / 2) - ((lines.length - 1) * lineSpacing / 2);
    
    for (let i = 0; i < lines.length; i++) {
        ctx.fillText(lines[i], canvas.width / 2, startY + (i * lineSpacing));
    }
    ...
    texture.repeat.set(1, 1); // Only 1 repeat so it doesn't wrap twice

    const geometry = new THREE.CylinderGeometry(0.6, 0.6, 1.5, 16, 1, true);
    ...
    lantern.scale.set(0.4, 0.4, 0.4); 
    
    // Pick a guaranteed clearly separated random slot
    if (availableSlots.length === 0) {
        availableSlots = [...lanternSlots];
    }
    const slotIndex = Math.floor(Math.random() * availableSlots.length);
    const slot = availableSlots.splice(slotIndex, 1)[0];
    
    // Add tiny jitter so it feels natural
    const spawnX = slot.x + (Math.random() * 0.2 - 0.1);
    const spawnY = slot.y + (Math.random() * 0.2 - 0.1);
    const spawnZ = 1.0;
    
    lantern.position.set(spawnX, spawnY, spawnZ);
    lantern.rotation.y = Math.PI; // Face the text directly to the camera
    
    // Optimize: Removed PointLight for massive performance boost
    ...
    lantern.userData = {
        ...
        startX: lantern.position.x,
        targetY: lantern.position.y 
    };
...
function animate() {
    ...
    for (let i = lanterns.length - 1; i >= 0; i--) {
        ...
        // Lock position in place with just a tiny breathing effect
        l.position.y = l.userData.targetY + Math.sin(l.userData.age * 0.01) * 0.05; 
        
        // Minimal edge drift
        l.position.x = l.userData.startX + Math.sin(l.userData.age * l.userData.swaySpeed + l.userData.swayOffset) * 0.1;
        
        // Keep rotation locked so text faces camera at all times, with microscopic tilt
        l.rotation.y = Math.PI + Math.sin(l.userData.age * 0.01) * 0.05; 
        l.rotation.z = Math.sin(l.userData.age * l.userData.swaySpeed + l.userData.swayOffset) * 0.02;
        l.rotation.x = 0;
    }
